// B-Unit (TVP — Transporte e Vendas de Peças) - Lógica da Aplicação & Catálogo de Peças Automóveis
...
const productsData = [
  {
    ...
    desc: "Conjunto completo de transmissão automóvel em excelente estado de conservação mecânica.",
    image: "assets/images/caixadeac.jpg",
    available: true
  },
  {
    id: 2,
    ...
    category: "Direção",
    desc: "Caixa de direção para substituição automóvel com elevado padrão de precisão e durabilidade.",
    image: "assets/images/CAIXA DE DIREÇÃO.jpg",
    available: true
  },
  {
    id: 3,
    ...
    desc: "Alternador elétrico de alta eficiência para sistema de carga de bateria e iluminação.",
    image: "assets/images/ALTERNADOR.jpeg",
    available: true
  },
  {
    id: 4,
    ...
    category: "Sistema elétrico",
    desc: "Motor de arranque de elevada fiabilidade e arranque rápido para motores a gasolina e diesel.",
    image: "assets/images/motor-de-arranque.jpg",
    available: true
  }
];
...
// Função de Sanitização de Segurança contra XSS e HTML Injection
...
// 2. Navegação entre as Páginas (SPA)
...
// 3. Renderização do Catálogo de Peças (Sem preços)
function renderProducts() {
  ...
  const filtered = productsData.filter(item => {
    const matchesCat = (currentCategory === "Todas as Categorias" || currentCategory === "Todos" || item.category === currentCategory);
  ...
  container.innerHTML = filtered.map(item => `
    <div class="dark-card p-5 flex flex-col justify-between group shadow-lg">
      <div class="relative h-56 bg-black rounded-xl overflow-hidden mb-4 border border-slate-800 flex items-center justify-center p-3">
        <img src="${sanitizeInput(item.image)}" alt="${sanitizeInput(item.name)}" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" loading="lazy">
        <span class="absolute top-3 right-3 bg-black/90 text-red-400 text-xs font-semibold px-3 py-1 rounded-full border border-red-500/40 shadow-md">
          ${sanitizeInput(item.category)}
        </span>
      </div>
      <div class="flex-1 flex flex-col justify-between">
        <div>
          <h3 class="text-lg font-bold text-white mb-2 leading-snug font-serif-heading">${sanitizeInput(item.name)}</h3>
        ...
        <div>
          ...
          <button onclick="openQuoteModal('${sanitizeInput(item.name)}')" class="w-full btn-red py-3 px-4 rounded-lg text-sm flex items-center justify-center gap-2 font-bold shadow-md cursor-pointer">
...
function initForms() {
  ...
  if (quoteForm) {
    quoteForm.addEventListener("submit", (e) => {
      ...
      const message = `*Solicitação de Cotação - B-Unit (TVP)*%0A%0A` +
  ...
  if (transportForm) {
    transportForm.addEventListener("submit", (e) => {
      ...
      const message = `*Solicitação de Informações - Transporte Escolar B-Unit*%0A%0A` +
                      ...
                      `*Contacto:* ${encodeURIComponent(phone)}%0A` +
                      `*Escola:* ${encodeURIComponent(school || 'Escola Arco-Íris')}%0A` +
